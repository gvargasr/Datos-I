...
class Grafos { 
   public:
	...
	void Kruskal();
	void algKruskal(listaDGrafo* grafo, listaDGrafo* AEM1);
	void MostrarKruskal();
...
void Grafos::cargaGrafo1() {
    ...
    Grafo1->Mostrar();
    cout<<"\n\n\t .:LISTA DE ADYACENCIA GRAFO #1:. "<<endl<<endl;
    MostrarListaAdj();
}
...
void Grafos::cargaGrafo2() {
    ...
    cout<<"\n\n\t .:LISTA DE ADYACENCIA GRAFO #2:. "<<endl<<endl;
    MostrarListaAdj2();
}
    ...
    while (!cola->ListaVacia()) {
        ...
        for (auto neighbor : listaAdj[nodoActual]) {
            if (visitado.find(neighbor.first) == visitado.end()) {
                ...
                visitado.insert(neighbor.first);
            }
        }
    }
    cout <<endl;
...
void Grafos::Kruskal(){
    ...
    if(largo == 0){
    	...
    	Menu();
            ...
            cout << "Generando Algoritmo de Kruskal: "<<endl<<endl;
            MostrarKruskal();
            cout<< "\nArchivo Kruskal.txt creado."<<endl<<endl;
    }
}


void Grafos::MostrarKruskal() {
    pnodoVertice aux = verticesKruskal->primero;
    lista* visitados = new lista();
    lista* destinos = new lista();
    int costoMinimo = 0;
	ofstream kruskal("Kruskal.txt");
	kruskal<<"\t .:AEM KRUSKAL:."<<endl<<endl;
    while (aux) {
        if (!visitados->Buscar(aux->origen) || !visitados->Buscar(aux->destino)) {
            visitados->InsertarFinal(aux->origen);
            visitados->InsertarFinal(aux->destino);
            destinos->InsertarFinal(aux->destino);
            cout << aux->origen << aux->destino << "-" << aux->costo << endl;
            costoMinimo = costoMinimo + aux->costo;
            kruskal<<aux->origen << aux->destino << "-" << aux->costo << endl;
        }else{
        	if(destinos->Buscar(aux->origen) && destinos->Buscar(aux->destino)){
			}
        	if(!destinos->Buscar(aux->origen) || !destinos->Buscar(aux->destino)){
        		visitados->InsertarFinal(aux->origen);
            	visitados->InsertarFinal(aux->destino);
        		destinos->InsertarFinal(aux->origen);
        	    destinos->InsertarFinal(aux->destino);
        		cout << aux->origen << aux->destino << "-" << aux->costo << endl;
        		costoMinimo = costoMinimo + aux->costo;
	            kruskal<<aux->origen << aux->destino << "-" << aux->costo << endl;
...
void Grafos::algKruskal(listaDGrafo* grafo, listaDGrafo* AEM1) {
	MostrarKruskal();
}
