...
listaDCompra::listaDCompra() { primero =  NULL; }

bool listaDCompra::ListaVacia() { return primero == NULL; }
...
string listaDCompra::Mostrar()
{
   ...
   string print;
  // while(aux) {
      cout<< "" << aux->cod << ":" << aux->nomProd << ":" << aux->kcal << ":" << aux->precio << ":" << aux->cantidad<<endl;
      print = to_string(aux->cod)+":"+aux->nomProd+":"+to_string(aux->kcal)+":"+to_string(aux->precio)+":"+to_string(aux->cantidad);
 //   aux = aux->siguiente;
 //  }
   cout << endl;
   return print;
...
int listaDCompra::CalcularPrecio(){
	int precio = 0;
	pnodoComp aux = primero;
	while (aux!=NULL){
		precio = precio + (aux->precio * aux->cantidad);
		aux = aux->siguiente;
	}
	return precio;
}
