#include <iostream>
#include "PilaProducto.cpp"
#include "NodoBinarioProducto.h"
#include "BinarioProducto.h"
#include <string>

using namespace std;

void NodoBinario::InsertaBinario(int num, string pais, int kcal, int price,int cant)
{
    if(num < valor){
        if(Hizq == NULL){
            Hizq = new NodoBinario(num, pais, kcal, price, cant);
        }else{
            Hizq->InsertaBinario(num, pais, kcal, price, cant);
        }
    }else{
        if(Hder == NULL){
            Hder = new NodoBinario(num, pais, kcal, price, cant);
        }else{
            Hder->InsertaBinario(num, pais, kcal, price, cant);
        }
    }
}

void Binario::InsertaNodo(int num, string pais, int kcal, int price,int cant)
{
    if(raiz == NULL){
        raiz = new NodoBinario(num, pais, kcal, price, cant);
    }else{
        raiz->InsertaBinario(num, pais, kcal, price, cant);
    }
}
...
void Binario::PostordenI(){
    ...
    while(!p.Vacia() || Act != NULL){
        if(Act != NULL){
            p2.Push(new NodoBinario(Act->valor, Act->producto, Act->caloria, Act->precio, Act->cantidad));
            p.Push(Act);
            Act = Act->Hder;
        }else{
            Act = p.Pop();
...
void Binario::Borrar(NodoBinario* &D, bool Hh)
{
    ...
    if(D->Hder != NULL){
        ...
        if(Hh){
            ...
            q->valor = D->valor;
            q->producto = D->producto;
            q->caloria = D->caloria;
            q->precio = D->precio;
            q->cantidad = D->cantidad;
            q = D;
            D = D->Hizq;
            Hh = true;
        }
...
void Binario::InsertarBalanceado(NodoBinario* &ra, bool &Hh, int x, string nombre, int kcal, int price, int cant){
    pNodoBinario n1;

    if(ra == NULL){
        ra = new NodoBinario(x, nombre, kcal, price, cant);
        cout << ra->valor << "q as" << endl;
        Hh = true;
    }else{
        cout << "q as2" << endl;
        if(x < ra->valor){
            InsertarBalanceado(ra->Hizq, Hh, x, nombre, kcal, price, cant);
            ...
            if(x > ra->valor){
                cout << "Hh:" << Hh << endl;
                InsertarBalanceado(ra->Hder, Hh, x, nombre, kcal, price, cant);
