...
//        cout << "La Producto: " << num << " no se encuentra" << endl;
    ...
    if (num < nodoB->valor) {
        return BuscarProducto(nodoB->Hizq, num);
    } else if (num > nodoB->valor) {
        return BuscarProducto(nodoB->Hder, num);
    } else {
    ...
    if(raiz != NULL){
        if(x < raiz->valor){
            ...
            if(q->Hder == NULL){
                ...
                NodoBinarioProducto* sucesor = MinValorNodo(q->Hder);
                ...
                // Elimina el sucesor
                BorrarBalanceadoProducto(q->Hder, Hh, sucesor->valor);
...
void AVLProducto::InsertarBalanceadoProducto(NodoBinarioProducto* &ra, bool &Hh, int x, string nombre, int cal, int precio, int cant){
    ...
    //    cout << "q as2" << endl;
        ...
        if(x < ra->valor){
            InsertarBalanceadoProducto(ra->Hizq, Hh, x, nombre,  cal,  precio,  cant);
            ...
            if(x > ra->valor){
           //     cout << "Hh:" << Hh << endl;
                InsertarBalanceadoProducto(ra->Hder, Hh, x, nombre,  cal,  precio,  cant);
...
NodoBinarioProducto* AVLProducto::ProductoMasBuscado(NodoBinarioProducto* nodoB) {
    ...
    InordenR(nodoB, masBuscado, maxContador);
	cout<< "\nEl producto mas buscado es: \n"<<masBuscado->valor<<":"<<masBuscado->Producto<<"\nFue buscado: "<<masBuscado->Contador<<" veces."<<endl<<endl;
    return masBuscado;
}

void AVLProducto::InordenR(NodoBinarioProducto* R, NodoBinarioProducto*& masBuscado, int& maxContador) {
    if (R == NULL) {
        return;
    }
...
void AVLProducto::InordenRCompras(NodoBinarioProducto* R, NodoBinarioProducto*& masBuscado, int& maxContador) {
    ...
    InordenRCompras(R->Hizq, masBuscado, maxContador);
    ...
    InordenRCompras(R->Hder, masBuscado, maxContador);
}
...
bool AVLProducto::BuscarProductoCantidadCompra(NodoBinarioProducto* nodoB, int num) {
    ...
    if (num < nodoB->valor) {
        return BuscarProductoCantidadCompra(nodoB->Hizq, num);
    } else if (num > nodoB->valor) {
        return BuscarProductoCantidadCompra(nodoB->Hder, num);
    } else {
